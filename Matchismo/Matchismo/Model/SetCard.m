...
+(uint) MAX_VARIATION
{  //changed from a const to a function so that the SetDeck class can access this
...
- (int)match:(NSArray *)otherCards
{
    ...
    if (otherCards.count ==2 ) // right number of cards to do a match check
    {
        ...
        if ([self.class isAllDifferentOrAllSameNumber1:self.number number2:setCard2.number number3:setCard3.number ])
            if ([self.class isAllDifferentOrAllSameNumber1:self.symbol number2:setCard2.symbol number3:setCard3.symbol ])
                if ([self.class isAllDifferentOrAllSameNumber1:self.shading number2:setCard2.shading number3:setCard3.shading ])
                    if ([self.class isAllDifferentOrAllSameNumber1:self.color number2:setCard2.color number3:setCard3.color ])
                        score = 1; //match
    }
